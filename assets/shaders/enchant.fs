...
number taxi_len(vec2 s)
{
    vec2 s2 = s - vec2(0.5, 0.5);
	return abs(s2.x) + abs(s2.y);
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number fac = max(min(2.*sin((taxi_len(90.*adjusted_uv) + enchant.r*2.) + 3.*(1.+0.8*cos(taxi_len(113.1121*adjusted_uv) - enchant.r*3.121))) - 1. - max(5.-taxi_len(90.*adjusted_uv), 0.), 1.), 0.);
    vec2 rotater = vec2(cos(enchant.r*0.1221), sin(enchant.r*0.3512));
    number angle = dot(rotater, adjusted_uv)/(taxi_len(rotater)*taxi_len(adjusted_uv));
    number fac2 = max(min(5.*cos(enchant.g*0.3 + angle*3.14*(2.2+0.9*sin(enchant.r*1.65 + 0.2*enchant.g))) - 4. - max(2.-taxi_len(20.*adjusted_uv), 0.), 1.), 0.);
